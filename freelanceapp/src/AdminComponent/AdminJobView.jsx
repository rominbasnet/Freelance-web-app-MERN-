...
const AdminJobView = ({ id, date, jobTitle, jobDescription, skillSetReq, jobBudget, business, interested}) =>{
  const dispatch = useDispatch();
  const [isDeleted, setIsDeleted] = useState(false);
  
  const Deleted = async(e, jobId) =>{
    e.preventDefault();
    //route delete garne
        try{
        const response = await axios.delete(`http://localhost:8000/adminmanage/deletejobbyid/${jobId}`);
        const deleted = response?.data?.isDeleted;
...
<div className="bg-laracast_bg border-gray-750 shadow hover:bg-laracast_hoverbg w-full max-w-4xl flex flex-col sm:flex-row gap-3 sm:items-center  justify-between px-5 py-4 mb-3 rounded-md">
    ...
    <div>
    <button onClick={(e)=>Deleted(e, id)} className="bg-sky-200 hover:bg-gray-100 text-gray-900 font-medium px-4 py-2 rounded-2xl flex gap-1 items-center">
     Delete
