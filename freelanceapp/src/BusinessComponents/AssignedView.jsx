...
import {AddRating} from '../actions/businessAction';
import {useDispatch} from 'react-redux';

const AssignedView = ({ id,  freelancer, website, status, githubUsername, skills, student, date, jobId, rated, workStatus, specialization, image}) =>{
  ...
  const [viewRating, setViewRating] = useState(rated);  
  ...
  const handleRatingSubmit = () => {
    dispatch(AddRating(id, jobId, rating));
    setViewRating(false)
  };
...
<div key={id} className="bg-laracast_bg border-gray-750 shadow hover:bg-laracast_hoverbg w-full max-w-4xl flex flex-col sm:flex-row gap-3 sm:items-center  justify-between px-5 py-4 mb-3 rounded-md">
    ...
    <div className="flex justify-between">
      <span className="text-sm font-mono text-posted_txt">Joined on {date.slice(0,10)}</span>
    
    {(!viewRating && workStatus === 'Completed') &&(
<div className="tooltip tooltip-open tooltip-bottom tooltip-success" data-tip="Rate freelancer">
    <div className="rating rating-sm">
    <input type="radio" name="rating-4"  value="9" className="mask mask-star-2 bg-yellow-500 hidden" onChange={(e)=>{e.preventDefault()}} checked/>
    <input type="radio" name="rating-4"  value="1" className="mask mask-star-2 bg-yellow-500" onChange={handleRatingChange} />
    <input type="radio" name="rating-4"  value="2" className="mask mask-star-2 bg-yellow-500" onChange={handleRatingChange} />
    <input type="radio" name="rating-4"  value="3" className="mask mask-star-2 bg-yellow-500" onChange={handleRatingChange} />
    <input type="radio" name="rating-4"  value="4" className="mask mask-star-2 bg-yellow-500" onChange={handleRatingChange} />
    <input type="radio" name="rating-4"  value="5" className="mask mask-star-2 bg-yellow-500" onChange={handleRatingChange} />
    </div>
    </div>
    )
    }
    ...
    <img className="rounded-full h-12 w-12" src={`http://localhost:8000/${image}`} alt="Freelancer Profile" />
      ...
      <span className="border border-green-300 mt-2 bg-skill_bg font-mono font-bold text-skill_border text-sm rounded-full px-3 py-1 ">
      {specialization}  
      </span>
    </div>
    ...
    {isRatingSelected && (
        <button className="bg-green-300 text-gray-900 rounded-full px-3 py-1 font-mono font-bold text-sm" onClick={handleRatingSubmit}>
        Submit Rating
        </button>
    )}
