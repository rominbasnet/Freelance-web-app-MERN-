  ...
  const jobCompleted = location.state?.jobCompleted; 
   ...
   useEffect(() =>{
    ...
    async function getRecommendation(id) {
      console.log(id)
      try {
        const response = await axios.get(`http://localhost:8000/job/recommend/${id}`);
...
) : profile.status === 404 || profile.length === 0 ? (
  <>
    <h1 className="text-gray-300">No Freelancers Found</h1>
  </>
) : (
  <>
    {profile?.length > 0 ? (
